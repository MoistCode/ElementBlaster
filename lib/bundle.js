...
var checkElementCollisions = function checkElementCollisions(els, players) {
  for (var i = 0; i < els.length; i++) {
    for (var j = 0; j < players.length; j++) {
      if (players[j].coordX < els[i].posX + 4 && players[j].coordX > els[i].posX - 4 && players[j].coordY < els[i].posY + 4 && players[j].coordY > els[i].posY - 4) {
        console.log(players[j]);
        _player.deletePlayer[players[j].id];
      }
    }
  }
};
