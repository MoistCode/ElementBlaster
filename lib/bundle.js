...
var _computerplayer = __webpack_require__(4);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  generatePlayers(gameContext, gameCanvas);
...
var checkElementCollisions = function checkElementCollisions(els, players, explosionPng, context) {
  ...
  for (var i = 0; i < els.length; i++) {
    for (var j = 0; j < players.length; j++) {
      if ((els[i].human == true && !(players[j] instanceof _humanplayer.HumanPlayer) || els[i].human == false && players[j] instanceof _humanplayer.HumanPlayer) && players[j].coordX < els[i].posX + 5.5 && players[j].coordX > els[i].posX - 5.5 && players[j].coordY < els[i].posY + 5.5 && players[j].coordY > els[i].posY - 5.5) {
...
var generatePlayers = function generatePlayers(context, canvas) {
  for (var i = 0; i < 5; i++) {
...
var generateResetButton = function generateResetButton(context, canvas) {
  context.beginPath();
...
var Element = function () {
  ...
  _createClass(Element, [{
    ...
    value: function checkForWall() {
      ...
      if (this.posX <= 625 / 1440 * this.canvas.width && this.posY <= 115 / 810 * this.canvas.height) {
        this.velocity = -this.velocity;
      } else if (this.posX >= 1404 / 1440 * this.canvas.width) {
        this.velocity = -this.velocity;
      } else if (this.posY <= 35 / 810 * this.canvas.height) {
        this.velocity = -this.velocity;
      } else if (this.posX <= 355 / 1440 * this.canvas.width) {
        this.velocity = -this.velocity;
      } else if (this.posY >= 765 / 810 * this.canvas.height) {
        this.velocity = -this.velocity;
      }
    }
  }]);
...
exports.deletePlayer = exports.getAllPlayers = exports.Player = undefined;
...
function drawGame(context, canvas) {
  context.beginPath();
  context.lineWidth = '5';
  context.strokeRect(330 / 1440 * canvas.width, 0 / 810 * canvas.height, 1100 / 1440 * canvas.width, 735 / 810 * canvas.height);
  context.stroke();
  context.closePath();
}
...
function drawRightSide(context, canvas) {
  context.beginPath();
  context.lineWidth = '5';
  context.strokeRect(10 / 1440 * canvas.width, 0 / 810 * canvas.height, 310 / 1440 * canvas.width, 735 / 810 * canvas.height);
...
function drawTitle(context, canvas) {
  ...
  context.fillText('BALL BLASTER', 55 / 1440 * canvas.width, 33 / 810 * canvas.height);
  context.closePath();
}

function drawRulesBorder(context, canvas) {
  context.beginPath();
  context.strokeRect(30 / 1440 * canvas.width, 50 / 810 * canvas.height, 270 / 1440 * canvas.width, 200 / 810 * canvas.height);
  context.fillStyle = '#FF0000';
  context.fill();
  context.closePath();
  context.font = '20px Verdana';
  context.fillText('Instructions', 118 / 1440 * canvas.width, 75 / 810 * canvas.height);

  var fontSize = Math.ceil(15 * (canvas.height / canvas.width * (1440 / 810)));

  context.font = fontSize + 'px Verdana';

  context.fillText('1. Shoot other circles.', 40 / 1440 * canvas.width, 100 / 810 * canvas.height);
  context.fillText('2. Dodge incoming projectiles', 40 / 1440 * canvas.width, 120 / 810 * canvas.height);
  context.fillText('3. Interrupt blast with an element stronger', 40 / 1440 * canvas.width, 140 / 810 * canvas.height);
  context.fillText('than the incoming projectile.', 55 / 1440 * canvas.width, 180 / 810 * canvas.height);

  context.fillText('4. Shoot other circles.', 40 / 1440 * canvas.width, 161 / 810 * canvas.height);
}
...
function drawSettingsBorder(context, canvas) {
  ...
  context.lineWidth = 5;
  context.strokeRect(35 / 1440 * canvas.width, 540 / 810 * canvas.height, 262 / 1440 * canvas.width, 225 / 810 * canvas.height);
  context.closePath();
}
...
function drawElementPowerBorder(context, canvas) {
  context.beginPath();
  context.lineWidth = '1';
  context.strokeRect(330 / 1440 * canvas.width, 1 / 810 * canvas.height, 270 / 1440 * canvas.width, 50 / 810 * canvas.height);
  context.stroke();
  context.closePath();
}
