import { drawOuterBorders, drawElements, generateElementBarImages } from './util.js';
...
import { getAllPlayers, deletePlayer } from './player.js';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let humanPlayer = new HumanPlayer(gameContext, gameCanvas);
  generatePlayers(gameContext, gameCanvas);

  let imageGenerated = generateElementBarImages(gameContext, gameCanvas);
  ...
  function update() {
    ...
    if (Object.values(getAllPlayers()).length > 1) {
      ...
      checkElementCollisions(els, Object.values(getAllPlayers()));
    }
  }
...
const checkElementCollisions = (els, players) => {
...
const generatePlayers = (context, canvas) => {
  for(let i = 0; i < 5; i++) {
